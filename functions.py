...
def MNIST_MAKE_DATA(current_working_path, sqrt_N0, P, Ptest, noise):
	...
	if os.path.exists(current_working_path + "/dataset"):
		cprint("\nDataset directory found.", "yellow")
		print("\tLoading dataset.")
	else:
		cprint("\nDataset directory not found.", "yellow")
	...
	print(f"\tDataset size (train): \t{x_train.size()} \t- {y_train.size()}")
	print(f"\tDataset size (test) : \t{x_test.size()} \t- {y_test.size()}")
...
def CIFAR_MAKE_DATA(current_working_path, sqrt_N0, P, Ptest, noise):
